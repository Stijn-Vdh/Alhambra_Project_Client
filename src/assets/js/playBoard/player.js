...
function loadOpponents(players) {
    ...
    const enemyPlayersHtml = document.querySelector('.EnemyPlayers');
    enemyPlayersHtml.innerHTML = "";
    players.forEach(player => {
        if (player['name'] !== getPlayerName()) {
            const EnemyCard = `<div class="EnemyCard" id="${player['name']}">
...
function loadOpponentAlhambra(player){
    const alhambra = document.querySelector(`#alhambra${player['name']}`);
    const sizeBoard = 7;
    const city = player["city"]["board"];

    alhambra.innerHTML = "";

    for(let row = 0; row < sizeBoard; row++){
        for (let col = 0; col < sizeBoard; col++){
            const html = `<div class="plateBuilding ${player['name']}grid${row}${col}"></div>`;
    ...
    for (let row = 0; row < city.length; row++){
        for (let col = 0; col < city[row].length; col++){
            if (city[row][col] !== null){
                const buildingGridHtml = document.querySelector(`.${player['name']}grid${row+((sizeBoard-city.length)/2)}${col+((sizeBoard-city.length)/2)}`);
